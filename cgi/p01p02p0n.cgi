...
if "p1" not in form or "p2" not in form:
    ...
    print('<h1 id="sequences_header">{0:g},{1:g},Pn TABLE&ordm. Black-Hole-Parabola at Pn={2}.</h1>'.format(p1,p2,pna0))
    print('<button onclick="window.history.back()">Go Back</button><br /><br />')
    print('<table id="sequences_table">') #Starting the second table
    print('<tr class="sequences_table" id="y_vertex"><th class="sequences_table" id="y_vertex">y&ordm_vertex</th>') #y_vertex
    for px in pn_range: 
       a,b,c = a0b0c0_set(p1,p2,px) 
       if a!=0:
           yv = y_vertex(a,b,c)
           if yv < 0:
    ...
    print('</tr><tr class="sequences_table" id="offset"><th>offset&ordm</th>') #offset
    for px in pn_range:
       a,b,c = a0b0c0_set(p1,p2,px) 
       if a!=0:
           f = offset(a,b,c)
    ...
    print('</tr><tr class="sequences_table" id="a"><th>a&ordm</th>') #a
    for px in pn_range:
       a = (p1-(2*p2)+px)/2
       print('<td class="sequences_table" id="a">{:1.6g}</td>'.format(a))

    print('</tr><tr class="sequences_table" id="b"><th>b&ordm</th>') #b
    for px in pn_range:
       b = (px-p1)/2
       print('<td class="sequences_table" id="b">{:1.6g}</td>'.format(b))

    print('</tr><tr class="sequences_table" id="c"><th>c&ordm</th>') #c
    for px in pn_range:
       c = p2
...
#########################################
    ...
    for px in pn_range_dneg:
        a,b,c = a0b0c0_set(p1,p2,px)
        den_neg[px], den_y0[px] = density(a,b,c,-qtd+1)
