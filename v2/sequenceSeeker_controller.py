...
class SequenceSeeker:
    ...
    def __init__(self, p1, p2, p3, p1_step=0, p2_step=0, p3_step=1, kp3=10, kp2=1000, kp1=0):
        ...
        self.kp3_range = range(int(p3 - kp3 * p3_step), int(p3 + kp3 * p3_step) + 1,  # Set the range of columns to
                               int(p3_step) or 1)                                     # -k * step >= P <= k * step
        self.kp2_range = range(int(p2 - kp2 * p2_step), int(p2 + kp2 * p2_step) + 1,  # using p*_step as step.
                               int(p2_step) or 1)
        self.kp1_range = range(int(p1 - (kp1 * p1_step)), int(p1 + kp1 * p1_step) + 1, p1_step or 1)
