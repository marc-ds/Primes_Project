...
class SequenceSeeker():
    ...
    def __init__(self, p1, p2, p3, kp3=10, kp2=1000):
        ...
        #self.kp3_range = range((int(p3-(kp3*p3_step))), int(p3+(kp3*p3_step))+1, int(p3_step) or 1)  # set the range of columns
        #self.kp2_range = range((int(p2-kp2*p2_step)), int(p2+kp2*p2_step)+1, int(p2_step) or 1)  # to -k*step >=0<= k*step
        #self.kp1_range = range((int(p1-kp1*p1_step)), int(p1+kp1*p1_step)+1, int(p1_step) or 1)  # and p step
